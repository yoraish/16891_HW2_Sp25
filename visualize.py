...
class Animation:
    def __init__(self, my_map, starts, goals, paths, k=2):
        ...
        self.artists = []
        self.agents = dict()
        self.agent_tails = dict()  # only store the recent k positions.
        ...
        for i in range(len(self.paths)):
            self.agent_tails[i] = []
            for tail_idx in range(2 * k):
                self.agent_tails[i].append(Circle((starts[i][0], starts[i][1]), 0.2 * ((2 * k - tail_idx) / (2 * k)),
                                                  facecolor=Colors[i % len(Colors)],
                                                  edgecolor='black', alpha=0.5 / (tail_idx + 1)))
                self.patches.append(self.agent_tails[i][tail_idx])
    ...
    def animate_func(self, t):
        for agent_idx in range(len(self.paths)):
            pos = self.get_state(t / 10, self.paths[agent_idx])
            self.agents[agent_idx].center = (pos[0], pos[1])
            self.agent_names[agent_idx].set_position((pos[0], pos[1] + 0.5))
            # Draw path.
            for k in range(self.k * 2):
                pos_tail = self.get_state(((t-(k+1)*5) / 10), self.paths[agent_idx])
                self.agent_tails[agent_idx][k].center = (pos_tail[0], pos_tail[1])
        ...
        for i in range(0, len(agents_array)):
            for j in range(i+1, len(agents_array)):
                ...
                for k in range(self.k):
                    pos_tail1 = np.array(self.agent_tails[i][k].center)
                    pos_tail2 = np.array(self.agent_tails[j][k].center)
                    if np.linalg.norm(pos1 - pos_tail2) < 0.7:
                        d1.set_facecolor('red')
                        d2.set_facecolor('red')
                        print("COLLISION! (agent-tail) ({}, {}) at time {}".format(i, j, t/10))
                    if np.linalg.norm(pos2 - pos_tail1) < 0.7:
                        d1.set_facecolor('red')
                        d2.set_facecolor('red')
                        print("COLLISION! (agent-tail) ({}, {}) at time {}".format(i, j, t/10))
